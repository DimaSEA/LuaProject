...
local BtnTweenAlpha = createHackButton("👻 Ghost Tween Coolant Alpha", Color3.fromRGB(41, 128, 185), 1)
local BtnTweenBeta  = createHackButton("👻 Ghost Tween Coolant Beta", Color3.fromRGB(41, 128, 185), 2)
local BtnClearMines = createHackButton("💣 Destroy All Landmine", Color3.fromRGB(231, 76, 60), 3)
local BtnBringPipes = createHackButton("🛠️ Bring Pipes (Stand Up)", Color3.fromRGB(39, 174, 96), 4)
local BtnBringBoxes = createHackButton("📦 Bring Junction Box", Color3.fromRGB(230, 126, 34), 5)
local BtnBringTrans = createHackButton("⚡ Bring External Transformer", Color3.fromRGB(142, 68, 173), 6)
local BtnSetCoord   = createHackButton("🔵 [Car TP] Set Coordinate", Color3.fromRGB(0, 150, 255), 7)
local BtnTpCar      = createHackButton("🔴 [Car TP] Teleport Car", Color3.fromRGB(255, 50, 50), 8)
local BtnCheckTemp  = createHackButton("🌡️ Check Reactor Temperature", Color3.fromRGB(44, 62, 80), 9)
...
-- [[ AMBIL JUNCTION BOX ]] --
BtnBringBoxes.MouseButton1Click:Connect(function()
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    ...
    if count > 0 then
        BtnBringBoxes.Text = "📦 Brought " .. count .. " Boxes! ✅"
        task.wait(1)
        BtnBringBoxes.Text = "📦 Bring Junction Box"
    else
        BtnBringBoxes.Text = "No Boxes Found! ❌"
        task.wait(1)
        BtnBringBoxes.Text = "📦 Bring Junction Box"
    end
end)

-- [[ AMBIL EXTERNAL TRANSFORMER ]] --
BtnBringTrans.MouseButton1Click:Connect(function()
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    ...
    for _, o in pairs(game:GetService("Workspace"):GetDescendants()) do
        if o.Name == "ExternalTransformer" then
            local offset = (count * spacing) - 4
            local newPos = basePos + (hrp.CFrame.RightVector * offset)
...
-- [[ SCROLL MENU TOMBOL ]] --
local ButtonMenuScroll = Instance.new("ScrollingFrame")
ButtonMenuScroll.Size = UDim2.new(1, -16, 1, -10)
ButtonMenuScroll.Position = UDim2.new(0, 8, 0, 5)
ButtonMenuScroll.BackgroundTransparency = 1
ButtonMenuScroll.CanvasSize = UDim2.new(0, 0, 0, 325) -- Batas scroll disesuaikan
ButtonMenuScroll.ScrollBarThickness = 4
ButtonMenuScroll.Parent = MainContainer
...
local BtnClearMines = createHackButton("💣 Destroy All Landmine", Color3.fromRGB(231, 76, 60), 3)
local BtnBringPipes = createHackButton("🛠️ Bring Pipes (Stand Up)", Color3.fromRGB(39, 174, 96), 4)
local BtnBringBoxes = createHackButton("📦 Bring Junction Box", Color3.fromRGB(230, 126, 34), 5)      -- JUNCTION BOX AMAN!
local BtnBringTrans = createHackButton("⚡ Bring External Transformer", Color3.fromRGB(142, 68, 173), 6) -- TRANSFORMER NEW!
local BtnSetCoord   = createHackButton("🔵 [Car TP] Set Coordinate", Color3.fromRGB(0, 150, 255), 7)
local BtnTpCar      = createHackButton("🔴 [Car TP] Teleport Car", Color3.fromRGB(255, 50, 50), 8)
...
BtnBringTrans.MouseButton1Click:Connect(function()
    ...
    if count > 0 then
        BtnBringTrans.Text = "⚡ Brought " .. count .. " Transformers! ✅"
        task.wait(1)
        BtnBringTrans.Text = "⚡ Bring External Transformer"
    else
        BtnBringTrans.Text = "No Transformers Found! ❌"
        task.wait(1)
        BtnBringTrans.Text = "⚡ Bring External Transformer"
    end
end)
